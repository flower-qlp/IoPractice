...
@Component
public class KingWebSocketHandler extends TextWebSocketHandler{
    ...
    private static final Logger log= LoggerFactory.getLogger(KingWebSocketHandler.class);


    //处理文本消息
    @Override
    public void handleTextMessage(WebSocketSession session, TextMessage message) {
        log.trace("用户 " + session.getAttributes().get(WEB_SOCKET_USERNAME) + " 发来一条信息：" + message);
        try {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        log.trace("用户 " + username + " 连接 WebSocket 成功......当前用户数量为:" + users.size());
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
        ...
        log.trace(username + " 退出......剩余在线用户数量:" + users.size());
